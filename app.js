...
app.get("/user/tweets/", authenticationToken, async (request, response) => {
  ...
  const getTweetIds = getTweetIdsArray.map((eachUser) => {
    return eachUser.user_id;
  });
  console.log(`${getTweetIds}`);
  const getTweetsNameQuery = `select name from user where user_id in (${getTweetIds});`;
  const getTweetsName = await database.all(getTweetsNameQuery);
  response.send(getTweetsName);
});
